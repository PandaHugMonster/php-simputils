...
class PHP {
	...
	public static function init(
		...
		if (PHP::isArrayCompatible($args)) {
			$args = PHP::box($args);
		}

		if (!$config) {
			$config = new AppInitConfig($args);
		} else {
			$config->code_root = $args->get('code_root') ?? debug_backtrace()[0]['file'];
			$config->working_dir = $args->get('working_dir') ?? $config->code_root;
		}
	...
	static function currentUrl($refresh = false) {
		...
		if (!static::$_cached_current_url || $refresh) {
			...
			$protocol = empty($serv['HTTPS']) || $serv['HTTPS']
				?'https'
				:'http';
	...
	static function pd(mixed ...$args) {
		$callback = CodeBlocksCacheIndex::getRedefinition(AppInitConfig::REDEF_PD);
		if ($callback && $callback !== AppInitConfig::REDEF_PD) {
	...
	public static function pr(...$args): void {
		$callback = CodeBlocksCacheIndex::getRedefinition(AppInitConfig::REDEF_PR);
		if ($callback && $callback !== AppInitConfig::REDEF_PR) {
			$callback(...$args);
		} else {
			foreach ($args as $arg) {
				print_r($arg);
